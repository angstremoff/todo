...
- Экспорт/импорт: кнопки «Экспорт»/«Импорт» рядом с заголовком пространств. Экспорт даёт JSON (можно поделиться или скопировать), импорт принимает этот JSON; режимы: «Добавить» (merge) или «Заменить» (replace).
