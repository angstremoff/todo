...
- Экспорт/импорт: кнопки «Экспорт»/«Импорт» рядом с заголовком пространств. Экспорт даёт JSON (можно поделиться или сохранить в `todo-backup.json`), импорт принимает этот JSON или читает файл. Режимы импорта: «Добавить» (merge) или «Заменить» (replace).
