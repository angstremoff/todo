...
- Экспорт/импорт: кнопки «Экспорт»/«Импорт» рядом с заголовком пространств. Экспорт даёт JSON (копирование/поделиться), импорт принимает этот JSON; режимы: «Добавить» (merge) или «Заменить» (replace).
